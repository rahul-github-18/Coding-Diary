...
export const userQueryService = {
  ...
  getSubmissions: async () => {
    const res = await fetch(`/api/user/submissions?t=${Date.now()}`, {
      headers: getAuthHeaders(),
      cache: 'no-store'
    });
    return handleResponse(res);
  ...
  markSubmissionAsRead: async (id) => {
    const res = await fetch(`/api/user/submissions/${id}/read`, {
      method: 'PUT',
      headers: getAuthHeaders()
    });
    return handleResponse(res);
...
export const adminLoginHistoryService = {
  getLoginHistory: async (userId = '') => {
    const url = userId 
      ? `/api/admin/login-history?userId=${userId}&t=${Date.now()}` 
      : `/api/admin/login-history?t=${Date.now()}`;
    const res = await fetch(url, {
      headers: getAuthHeaders(),
      cache: 'no-store'
    });
